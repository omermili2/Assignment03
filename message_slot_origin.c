...
int newChannel(struct msg_slot* slot, unsigned long channel_id) {
    ...
    if ((findChannel(slot, channel_id)) == NULL) {  /* if channel does not exist */
        ...
        if(!(new_channel->msg = kmalloc(sizeof(char) * BUF_LEN, GFP_KERNEL))) { /* channel's msg memory allocation */
            kfree(channel);
            return -ENOMEM;                         /* kmalloc failed */
        }
        ...
        new_channel->next = slot->channenels_head;  /* assign current head as new channel's next */
...
static int __init simple_init(void) {
    ...
    printk("Registration successful with major number %d.\n", MAJOR_NUM );
